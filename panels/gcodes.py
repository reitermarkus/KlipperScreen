...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        for name, val in self.sort_items.items():
            s = self._gtk.Button(None, val, f"color{n % 4 + 1}", 0.5, Gtk.PositionType.RIGHT, 1)
    ...
    def confirm_delete_directory(self, widget, dirpath):
        ...
        self._screen._confirm_send_action(
            None, _("Delete Directory?") + "\n\n" + dirpath, "server.files.delete_directory", params
        )
    ...
    def change_sort(self, widget, key):
        ...
        self.labels[f"sort_{key}"].set_image(
            self._gtk.Image(self.sort_icon[self.sort_current[1]], self._gtk.img_scale * self.bts)
        )
        ...
        self._config.set(
            "main", "print_sort_dir", f"{key}_{'asc' if self.sort_current[1] == 0 else 'desc'}"
        )
        self._config.save_user_config_options()
    ...
    def confirm_print(self, widget, filename):
        ...
        orientation = (
            Gtk.Orientation.VERTICAL if self._screen.vertical_mode else Gtk.Orientation.HORIZONTAL
        )
        ...
        if self._screen.vertical_mode:
            ...
            height = (
                self._screen.height - self._gtk.dialog_buttons_height - self._gtk.font_size * 5
            ) * 0.45
        else:
    ...
    def get_file_info_extended(self, fileinfo):
        ...
        if "modified" in fileinfo:
            ...
            if self.time_24:
                ...
                info += (
                    f":<b> {datetime.fromtimestamp(fileinfo['modified']):%Y/%m/%d %I:%M %p}</b>\n"
                )
        if "layer_height" in fileinfo:
            info += _("Layer Height") + f": <b>{fileinfo['layer_height']}</b> " + _("mm") + "\n"
        ...
        if "nozzle_diameter" in fileinfo:
            info += (
                _("Nozzle diameter") + f": <b>{fileinfo['nozzle_diameter']}</b> " + _("mm") + "\n"
            )
        if "slicer" in fileinfo:
            info += (
                _("Slicer") + f": <b>{fileinfo['slicer']} "
